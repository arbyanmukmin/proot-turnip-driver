...
build_mesa() {
    ...
    meson setup "$BUILD_DIR" --prefix /usr --libdir "$LIBDIR" \
        ...
        -D freedreno-kmds=msm,kgsl -D dri3=enabled -D buildtype=release -D glx=disabled \
        -D egl=enabled -D gles1=disabled -D gles2=disabled -D gallium-xa=disabled \
        -D opengl=false -D shared-glapi=false -D b_lto=true -D b_ndebug=true -D cpp_rtti=false \
        -D gbm=enabled -D llvm=disabled -D shared-llvm=disabled -D xmlconfig=disabled \
        -D buildtype=release || {
